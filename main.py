...
@bot.tree.command(name="threaten_creator", description="[LIMITED EDITION] Sends a /threaten result to this bot's creator, @zytronium. Go wild.")
@with_error_handling()
async def threaten_creator(interaction: Interaction):
    ...
    try:
        ...
        if threat == "Mods, banish him to Lowes." and randint(1, 2) == 2:
            # Send a ban command.
            await creator.send(f"-# Threat from {interaction.user.mention}\n"
                               f"?ban {creator.mention} banished to Lowes for being too low tier.")
        else:
            # Send the threat and store the message so we can edit it
            message = await creator.send(f"-# Threat from {interaction.user.mention}\n"
            ...
            if threat == "*Congratulations!* You just won a free vacation!":
                await asyncio.sleep(2.5)
                await message.edit(content=message.content + " **To hell.**")
        await interaction.response.send_message("Threat delivered to Deathbot's creator.")
    except Exception as err:
        await interaction.response.send_message("Failed to send the message to the creator. Did he block me?", ephemeral=True)
        raise err
